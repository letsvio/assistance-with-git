...
Жизненный цикл файлов:
    A[Untracked\n(неотслеживаемый)] -->|"git add\n(начать отслеживать)"| B[Staged\n(в индексе)]
    B -->|"git commit\n(сохранить в репозитории)"| C[Committed\n(в репозитории)]
    C -->|"изменения в файле\n(редактирование)"| D[Modified\n(изменённый)]
    D -->|"git add\n(подготовить к коммиту)"| B[Staged\n(в индексе)]
    B -->|"git reset HEAD <файл>\n(убрать из индекса)"| D[Modified\n(изменённый)]
    D -->|"git restore <файл>\n(отменить изменения)"| C[Committed\n(в репозитории)]
    A -->|"файл удалён из рабочей директории"| X[Deleted\n(удалённый)]
    D -->|"git checkout -- <файл>\n(откатить до последнего коммита)"| C[Committed\n(в репозитории)]
```
