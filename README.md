...
```mermaid
flowchart LR
    A["Untracked<br/>(неотслеживаемый)"] -->|git add<br/>(начать отслеживать)| B["Staged<br/>(в индексе)"]
    B -->|git commit<br/>(сохранить в репозитории)| C["Committed<br/>(в репозитории)"]
    C -->|изменение в файле<br/>(редактирование)| D["Modified<br/>(изменённый)"]
    D -->|git add<br/>(подготовить к коммиту)| B
    B -->|git reset HEAD &lt;файл&gt;<br/>(убрать из индекса)| D
    D -->|git restore &lt;файл&gt;<br/>(отменить изменения)| C
    A -->|файл удалён из рабочей директории| X["Deleted<br/>(удалённый)"]
    D -->|git checkout -- &lt;файл&gt;<br/>(откатить до последнего коммита)| C
```
